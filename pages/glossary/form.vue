...
export default {
  ...
  async created() {
    if(this.$route.query.id) {
      ...
      this.getGlossary(this.id)
    } 
  },
  methods: {
    async getGlossary(id) {
      const res = await this.apiGet("glossaries/" + id)
    ...
    async actionSave() {
      ...
      if(!this.id) {
        ...
        res = await this.apiPost("glossaries", postData);
        
      } else {
        // postData.updated_by = this.getUser().id
        postData = JSON.parse(JSON.stringify(postData))
        res = await this.apiPatch("glossaries/" + this.id, postData);
      }
